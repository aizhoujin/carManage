<template>
  <div id="app-root" class="box">
    ...
    <LayoutContent v-if="num" :index-data="indexData" title="接口性能测试" />
    <Productfunction
      v-if="flag"
      :product-features="productFeatures"
      :content-title="contentTitle"
    />
    <Productadvangtage v-if="flags" :product-advantage="productAdvantage" />
...
import Productfunction from '../../productService/components/ProductFunction.vue';
import Productadvangtage from '../publicComponent/ProductAdvantage3.vue';
...
const initData = async (key: string) => {
  ...
  if (res) {
    ...
    res.result.forEach((item: any) => {
      if (item.keyword === 'interface_performance_test') {
        Object.assign(indexData, item.paperList);
        num.value = true;
      } else if (item.keyword === 'interface_performance_test_feature') {
        Object.assign(productFeatures, item.paperList);
        flag.value = true;
      } else if (item.keyword === 'interface_performance_test_advantage') {
        Object.assign(productAdvantage, item.paperList);
        flags.value = true;
      } else if (item.keyword === 'interface_performance_test_scene') {
        Object.assign(applicableScene, item.paperList);
        flagss.value = true;
      } else if (item.keyword === 'interface_performance_test_process') {
...
initData('interface_performance_test_index');
</script>
