<template>
  <div id="app-root" class="box">
    ...
    <LayoutContent v-if="num" :index-data="indexData" title="测试过程管理" />
...
import UsageScenario from '../publicComponent/UsageScenario4.vue';
...
const initData = async (key: string) => {
  ...
  if (res) {
    ...
    res.result.forEach((item: any) => {
      if (item.keyword === 'test_procedure_management') {
        Object.assign(indexData, item.paperList);
        num.value = true;
      } else if (item.keyword === 'test_procedure_management_feature') {
        Object.assign(productFeatures, item.paperList);
        flag.value = true;
      } else if (item.keyword === 'test_procedure_management_advantage') {
        Object.assign(productAdvantage, item.paperList);
        flags.value = true;
      } else if (item.keyword === 'test_procedure_management_scene') {
        Object.assign(applicableScene, item.paperList);
        flagss.value = true;
      } else if (item.keyword === 'test_procedure_management_process') {
...
initData('test_procedure_management_index');
</script>
