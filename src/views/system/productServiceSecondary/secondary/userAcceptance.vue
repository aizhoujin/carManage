<template>
  <div id="app-root" class="box">
    ...
    <LayoutContent v-if="num" :index-data="indexData" title="用户验收测试" />
...
const initData = async (key: string) => {
  ...
  if (res) {
    res.result.forEach((item: any) => {
      if (item.keyword === 'user_acceptance_test') {
        Object.assign(indexData, item.paperList);
        num.value = true;
      } else if (item.keyword === 'user_acceptance_test_content') {
        Object.assign(productFeatures, item.paperList);
        flag.value = true;
      } else if (item.keyword === 'user_acceptance_test_advantage') {
        Object.assign(productAdvantage, item.paperList);
        flags.value = true;
      } else if (item.keyword === 'user_acceptance_test_scene') {
        Object.assign(applicableScene, item.paperList);
        flagss.value = true;
      } else if (item.keyword === 'user_acceptance_test_process') {
...
initData('user_acceptance_test_index');
</script>
