<template>
  <div id="app-root" class="box">
    ...
    <LayoutContent v-if="num" :index-data="indexData" title="质量管控方案咨询" />
...
const initData = async (key: string) => {
  ...
  if (res) {
    res.result.forEach((item: any) => {
      if (item.keyword === 'quality_and_testing') {
        Object.assign(indexData, item.paperList);
        num.value = true;
      } else if (item.keyword === 'quality_and_testing_content') {
        Object.assign(productFeatures, item.paperList);
        flag.value = true;
      } else if (item.keyword === 'quality_and_testing_advantage') {
        Object.assign(productAdvantage, item.paperList);
        flags.value = true;
      } else if (item.keyword === 'quality_and_testing_scene') {
        Object.assign(applicableScene, item.paperList);
        flagss.value = true;
      } else if (item.keyword === 'quality_and_testing_process') {
...
initData('quality_and_testing_index');
</script>
