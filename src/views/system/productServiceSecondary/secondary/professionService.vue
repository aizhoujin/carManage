<template>
  <div id="app-root" class="box">
    ...
    <LayoutContent v-if="num" :index-data="indexData" title="性能专家服务" />
...
import Productadvantage from '../publicComponent/ProductAdvantage2.vue';
import UsageScenario from '../publicComponent/UsageScenario2.vue'; //使用场景：3个
...
const initData = async (key: string) => {
  ...
  if (res) {
    ...
    res.result.forEach((item: any) => {
      if (item.keyword === 'performance_specialist_service') {
        Object.assign(indexData, item.paperList);
        num.value = true;
      } else if (item.keyword === 'performance_specialist_service_content') {
        Object.assign(productFeatures, item.paperList);
        flag.value = true;
      } else if (item.keyword === 'performance_specialist_service_advantage') {
        Object.assign(productAdvantage, item.paperList);
        flags.value = true;
      } else if (item.keyword === 'performance_specialist_service_scene') {
        Object.assign(applicableScene, item.paperList);
        flagss.value = true;
      } else if (item.keyword === 'performance_specialist_service_process') {
...
initData('performance_specialist_service_index');
</script>
