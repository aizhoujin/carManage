<template>
  <div id="app-root" class="box">
    ...
    <LayoutContent
      ...
      title="系统性能画像"
      url="/dashboard/portrait/testActivity"
    />
...
import SpecifRecommand from '../publicComponent/SpecifRecommand3.vue';
...
const getRecommandData = async () => {
  const [res] = await proxy.slAwait(getRecommand(4));
  if (res) {
    console.log(res);
...
const initData = async (key: string) => {
  ...
  if (res) {
    ...
    res.result.forEach((item: any) => {
      if (item.keyword === 'system_performance_profile') {
        Object.assign(indexData, item.paperList);
        num.value = true;
      } else if (item.keyword === 'system_performance_profile_feature') {
        Object.assign(productFeatures, item.paperList);
        flag.value = true;
      } else if (item.keyword === 'system_performance_profile_advantage') {
        Object.assign(productAdvantage, item.paperList);
        flags.value = true;
      } else if (item.keyword === 'system_performance_profile_scene') {
        Object.assign(applicableScene, item.paperList);
        flagss.value = true;
      } else if (item.keyword === 'system_performance_profile_process') {
...
initData('system_performance_profile_index');
</script>
