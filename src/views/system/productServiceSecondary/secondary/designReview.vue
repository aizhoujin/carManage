<template>
  <div id="app-root" class="box">
    ...
    <LayoutContent v-if="num" :index-data="indexData" title="设计方案专家咨询" />
...
const initData = async (key: string) => {
  ...
  if (res) {
    ...
    res.result.forEach((item: any) => {
      if (item.keyword === 'design_proposal_expert_consultation') {
        Object.assign(indexData, item.paperList);
        num.value = true;
      } else if (item.keyword === 'design_proposal_expert_consultation_content') {
        Object.assign(productFeatures, item.paperList);
        flag.value = true;
      } else if (item.keyword === 'design_proposal_expert_consultation_advantage') {
        Object.assign(productAdvantage, item.paperList);
        flags.value = true;
      } else if (item.keyword === 'design_proposal_expert_consultation_scene') {
        Object.assign(applicableScene, item.paperList);
        flagss.value = true;
      } else if (item.keyword === 'design_proposal_expert_consultation_process') {
...
initData('design_proposal_expert_consultation_index');
</script>
